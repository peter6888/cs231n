...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    layer1 = X.dot(W1) + b1
    ReLu = np.maximum(0, layer1) # ReLu.shape -> (N,H)
    ...
    grads['W2'] += 2 * reg * W2 # regularization gradient
    
    dReLu = np.zeros_like(ReLu)
    dReLu[layer1 > 0] = 1
    delta = dscores.dot(W2.T) * dReLu
    grads['W1'] = X.T.dot(delta)
    grads['b1'] = np.sum(delta, axis=0) 
    grads['W1'] += 2 * reg * W1
